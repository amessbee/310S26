...
\begin{enumerate}
    \item Show that the naive approach of merging arrays pairwise (merge array 1 with 2, then merge 
    result with 3, etc.) takes $O(nk)$ time in the worst case.
    \item Describe an algorithm using a \textbf{min-heap} of size $k$ that merges all arrays in 
    $O(n \log k)$ time.
\end{enumerate}
...
\item \textit{The $k$-th smallest element problem arises in order statistics, 
database query optimization, and approximate algorithms. We designed a linear-time algorithm 
using the median-of-medians earlier, 
but a simpler $O(n \log k)$ algorithm using a heap is at times more practical.}
...
\begin{enumerate}
    ...
    \item Show that BuildHeap gives an $O(n)$ algorithm to find the minimum element. 
    Can you find the second-minimum in $O(\log n)$ additional time? 
    The $k$-th minimum in $O(k \log n)$ time?
    \item Suppose the array is already a \emph{max-heap}. What is the minimum number of elements
    you must examine to find the $k$-th largest element?
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textit{Proving the binary counter analysis with the accounting method by connecting bit flips to the number of 1-bits in the counter.}
    
    Consider a $k$-bit binary counter that starts at 0. Use the \textbf{accounting method} with a ``bank balance'' to prove that $n$ \texttt{Increment} operations cost $O(n)$ total, by reserving 2 bit flips per increment.
    
    \begin{enumerate}
        \item Prove that after each \texttt{Increment} operation, the bank balance equals the number of 1-bits in the current counter value. \textit{(Hint: Track how the bank balance changes: when we spend bits to flip 0-bits to 1s and 1-bits to 0s, and when we reserve new credits, the bank balance should always equal the number of 1s.)}
        \item Conclude that the total cost of $n$ increments is at most $2n$ bit flips.
        % \item Explain how this proof by accounting differs from the potential method analysis, and what role the ``bank balance = number of 1s'' invariant plays.
    \end{enumerate}
\end{enumerate}
