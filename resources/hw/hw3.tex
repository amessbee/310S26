	...
	\begin{center}
		\framebox{
			\vbox{
				...
				\vspace{2mm}
				\hbox to 6.28in { {\it Professor: #4 \hfill Deadline: Mar 16, 2026.} }
			}
		}
	\end{center}
	\vspace*{4mm}
...
% ---------------------------------------------------------------
% PROBLEM 1 — UNION-FIND TRACE  [30 points]
% ---------------------------------------------------------------

\item \textbf{[30 points]} \textit{Union-Find models dynamic connectivity, which arises in network routing, image segmentation, and Kruskal's MST algorithm.}
...
\begin{enumerate}[label=(\alph*)]
    \item \textbf{[12 pts]} Perform the following operations in order using union by rank.
    ...
    \item \textbf{[12 pts]} Now perform \texttt{Find}$(8)$ with path compression.
    Write down the sequence of nodes visited while traversing parent pointers from $8$ to the root.
    Show the updated parent array after path compression, and draw the resulting forest.
...
% ---------------------------------------------------------------
% PROBLEM 2 — RUNNING MEDIAN WITH TWO HEAPS  [35 points]
% ---------------------------------------------------------------

\item \textbf{[35 points]} \textit{The running median is used in streaming data analysis, real-time signal processing, and financial tick data.}

You receive integers one at a time in a stream. After each insertion you must report the \textbf{median} of all integers seen so far.  (If the count is even, report the average of the two middle values; if odd, report the middle value.)

\begin{enumerate}[label=(\alph*)]
    \item \textbf{[10 pts]} Describe a data structure using exactly \textbf{two heaps} — a max-heap $H_{\text{lo}}$ storing the smaller half of the elements, and a min-heap $H_{\text{hi}}$ storing the larger half — that supports \texttt{Insert} and \texttt{GetMedian} each in $O(\log n)$ time.
    Your answer must specify: (i)~the \textbf{size/ordering invariant} between $H_{\text{lo}}$ and $H_{\text{hi}}$; (ii)~the \textbf{Insert} procedure including when and how to rebalance; and (iii)~how \textbf{GetMedian} uses the heap roots to return the answer in $O(1)$.

    \item \textbf{[5 pts]} Prove that your invariant is maintained after every \texttt{Insert}.
    You may assume $H_{\text{lo}}$ and $H_{\text{hi}}$ each support \texttt{Insert} and \texttt{ExtractMax}/\texttt{ExtractMin} in $O(\log n)$.

    \item \textbf{[20 pts]} Trace your data structure on the following input stream:
    \[
        5,\quad 3,\quad 8,\quad 1,\quad 9,\quad 2,\quad 7
    \]
    After \emph{each} insertion, fill in one row of the table below.

    \bigskip
    \begin{center}
    \begin{tabular}{c|l|l|c}
    \hline
    \textbf{Inserted} & \textbf{$H_{\text{lo}}$ (max-heap, top first)} & \textbf{$H_{\text{hi}}$ (min-heap, top first)} & \textbf{Median} \\
    \hline
    5 & & & \\[6pt]
    3 & & & \\[6pt]
    8 & & & \\[6pt]
...
% ---------------------------------------------------------------
% PROBLEM 3 — QUEUE FROM TWO STACKS  [35 points]
% ---------------------------------------------------------------

\item \textbf{[35 points]} \textit{Implementing a queue from two stacks is a classic application of amortized analysis.}
...
\begin{enumerate}[label=(\alph*)]
    \item \textbf{[12 pts]} Trace the data structure on the following sequence of operations.
    ...
    \item \textbf{[23 pts]} First, explain why a single \texttt{Dequeue} on a queue holding $n$ elements can cost $\Theta(n)$ in the worst case.
    Then use the \textbf{accounting method} to prove that any sequence of $n$ \texttt{Enqueue} and \texttt{Dequeue} operations costs $O(n)$ total.
    Charge \textbf{3 credits} per \texttt{Enqueue} and \textbf{1 credit} per \texttt{Dequeue}, and maintain the \textbf{credit invariant}: every element currently in $S_{\text{in}}$ holds exactly 2 banked credits.
...
% ---------------------------------------------------------------
% PROBLEM 4 — KRUSKAL'S ALGORITHM: TRACE AND ROBUSTNESS  [30 points]
% ---------------------------------------------------------------

\bigskip
\item \textbf{[30 points]} \textit{Running Kruskal's algorithm by hand builds intuition for the cut property and reveals how sensitive the MST is to edge weight changes.}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item \textbf{[16 pts]} List all 9 edges in non-decreasing order of weight (break ties by smaller endpoint first), then trace Kruskal's algorithm.  Fill in every row of the table.  In the ``Action'' column write \textbf{Add} or \textbf{Skip}.  In the ``Reason'' column, for a \textbf{Skip} name the cycle that would have been created; for an \textbf{Add} state which two components merge.
