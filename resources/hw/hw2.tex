...
\item \textit{Write an algorithm to find the number of connected components in a graph.}

\item \textit{BFS and DFS}

Consider the following undirected graph with vertices: $A, B, C, D, E, F$.
\begin{center}

\begin{tikzpicture}[scale=1.0, every node/.style={circle, draw, minimum size=0.9cm}]
    % Nodes
    \node (A) at (0,2) {A};
    \node (B) at (2,3) {B};
    \node (C) at (2,1) {C};
...
\begin{enumerate}
    \item Start BFS from vertex $A$.  
    Write the order in which vertices are **discovered** (added to the queue).
    
    \item In the BFS tree, what is the **minimum number of edges** required to reach vertex $F$ from vertex $A$?
    
    \item Perform a DFS starting from vertex $A$.  
    Rule: Always explore the **alphabetically first unvisited neighbor**.  
    Trace your recursion: $A \to B \to \dots$
    
    Write the full **DFS discovery order**.
    
    \item Which vertex was discovered **last** in BFS?
    
    \item Which vertex was discovered **last** in DFS?
    
    \item In general, which algorithm is better for finding **shortest paths in unweighted graphs**? Explain why.
\end{enumerate}
