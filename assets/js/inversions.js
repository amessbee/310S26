// Inversion Counting Visualizer (Divide & Conquer)
(function () {
  // DOM
...
(function () {
  // Theme from main.js will initialize. Keep utilities local here.
  const barsEl = document.getElementById("bars");
  const invEl = document.getElementById("inv-count");
  const stepEl = document.getElementById("step-count");
  const sizeRange = document.getElementById("size");
  ...
  function highlightLine(n) {
    if (!pcEl) return;
    if (activeLine === n) return;
    const items = pcEl.querySelectorAll(".line");
  ...
  function randArray(n) {
    const arr = [];
    const used = new Set();
    // Use unique values to make inversions clearer; range 1..99
    while (arr.length < n) {
      const v = Math.floor(Math.random() * 99) + 1;
      if (!used.has(v)) {
        used.add(v);
  ...
  function getCurrentSegment() {
    if (lastHighlights.seg) return lastHighlights.seg;
  ...
  function stepOver() {
    ...
    const next = actions[actionIndex];
    if (next && next.type === "call") {
      // Jump to matching afterCall for this segment and call side
      let target = -1;
      for (let i = actionIndex; i < actions.length; i++) {
        const a = actions[i];
        if (
  ...
  // Build actions for mergesort-based inversion counting.
  function buildActions(a) {
    const acts = [];
    function mergeSort(l, r) {
      if (r - l <= 1) {
      ...
      const R = mergeSort(m, r);
      acts.push({ type: "afterCall", l, r, which: "R" });
      // merge with actions writing back into a[l..r)
      let i = 0, j = 0, k = l;
      acts.push({ type: "initmerge", l, r });
      while (i < L.length && j < R.length) {
        // compare
        acts.push({ type: "compare", l, r, i: l + i, j: m + j, vi: L[i], vj: R[j] });
        if (L[i] <= R[j]) {
          acts.push({ type: "write", l, r, k, value: L[i], invDelta: 0, tail: null });
          a[k++] = L[i++];
        } else {
          const invDelta = L.length - i;
          acts.push({ type: "write", l, r, k, value: R[j], invDelta, tail: null });
          a[k++] = R[j++];
        }
      }
      while (i < L.length) {
        acts.push({ type: "write", l, r, k, value: L[i], invDelta: 0, tail: "L" });
        a[k++] = L[i++];
      }
      while (j < R.length) {
        acts.push({ type: "write", l, r, k, value: R[j], invDelta: 0, tail: "R" });
        a[k++] = R[j++];
      }
      acts.push({ type: "done", l, r });
      return a.slice(l, r);
