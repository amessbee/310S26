...
(function () {
  ...
  const locals = { n: null, m: null, xL: null, xR: null, yL: null, yR: null, z0: null, z1: null, z2: null };
  ...
  function fromDigits(arr) { return Number(arr.join("")); }
  function addDigits(a, b) {
    const n = Math.max(a.length, b.length);
    const A = a.slice(); const B = b.slice();
    while (A.length < n) A.unshift(0);
    while (B.length < n) B.unshift(0);
    const out = []; let carry = 0;
    for (let i = n - 1; i >= 0; i--) {
      const s = A[i] + B[i] + carry; out.unshift(s % 10); carry = Math.floor(s / 10);
    }
    if (carry) out.unshift(carry);
    return out;
  }
  function splitDigits(arr, m) {
    const n = arr.length; const low = arr.slice(n - m); const high = arr.slice(0, n - m);
    return { high, low };
  }
  ...
  function renderBars() {
    ...
    function renderRow(rowEl, digits, splitM) {
      ...
      digits.forEach((d, idx) => {
        ...
        const lab = document.createElement("div"); lab.className = "label"; lab.textContent = String(d); el.appendChild(lab);
        rowEl.appendChild(el);
      });
      // split overlay
      if (typeof splitM === "number") {
        const seg = document.createElement("div"); seg.className = "segment";
        const n = digits.length; const leftPct = ((n - splitM) / n) * 100; const widthPct = (splitM / n) * 100;
        const box = document.createElement("div"); box.className = "range"; box.style.left = leftPct + "%"; box.style.width = "calc(" + widthPct + "% - 6px)"; seg.appendChild(box);
  ...
  function buildActionsKaratsuba(xArr, yArr) {
    ...
    function kMult(xD, yD) {
      ...
      acts.push({ type: "combine", m, z0: fromDigits(z0), z2: fromDigits(z2), z1raw: fromDigits(z1raw) });
      // z1 = z1raw - z0 - z2
      const z1 = fromDigits(z1raw) - fromDigits(z0) - fromDigits(z2);
      const res = (fromDigits(z2) * Math.pow(10, 2 * m)) + (z1 * Math.pow(10, m)) + fromDigits(z0);
  ...
  // Playback
  function startAuto() {
    if (autoTimer) return; const delay = clamp(Number(speedRange.value) || 500, 100, 1500);
  ...
  function stepOnce() {
    ...
    if (act.type === "seg") {
      highlights.segN = act.n; highlights.splitM = null; resEl.textContent = ""; renderLocals({ n: act.n, m: null, z0: null, z1: null, z2: null }); highlightLine(2);
    } else if (act.type === "base") {
      renderLocals({ n: 1, m: 0 }); highlightLine(3); resEl.textContent = String(act.prod);
    } else if (act.type === "split") {
      highlights.splitM = act.m; renderLocals({ m: act.m }); highlightLine(4);
    } else if (act.type === "call") {
      const lineMap = { z0: 7, z2: 8, z1: 9 }; highlightLine(lineMap[act.which] || 7);
    } else if (act.type === "afterCall") {
      // noop visual, locals updated during combine
    } else if (act.type === "combine") {
      renderLocals({ z0: act.z0, z2: act.z2 }); highlightLine(10);
    } else if (act.type === "result") {
      resEl.textContent = String(act.value); renderLocals({ z1: null }); highlightLine(10);
    }
    renderBars();
  ...
  function stepOver() {
    ...
    for (let i = actionIndex; i < actions.length; i++) {
      const a = actions[i]; if (a.type === "combine") { targetIndex = i; break; }
    }
    if (targetIndex === -1) return stepOnce();
    stopAuto(); while (actionIndex < targetIndex) { if (!stepOnce()) break; }
    return stepOnce();
  }

  // Wire UI
  digitsRange.addEventListener("input", () => { digitsNum.value = digitsRange.value; regenRandom(); });
  digitsNum.addEventListener("change", () => { digitsRange.value = digitsNum.value; regenRandom(); });
  function regenRandom() {
    const d = clamp(Number(digitsRange.value) || 6, 1, 12);
    const xMax = Math.pow(10, d) - 1; const yMax = xMax;
    const x = Math.floor(Math.random() * xMax);
    const y = Math.floor(Math.random() * yMax);
    xInput.value = String(x); yInput.value = String(y);
    resetAll();
  }
  ...
  function resetAll() {
    ...
    xDigits = toDigits(x, d); yDigits = toDigits(y, d);
    actions = buildActionsKaratsuba(xDigits.slice(), yDigits.slice());
    actionIndex = 0; resEl.textContent = ""; stepEl.textContent = "0"; highlights.splitM = null; activeLine = null;
    highlightLine(1); renderBars(); renderLocals({ n: Math.max(xDigits.length, yDigits.length), m: null, z0: null, z1: null, z2: null });
  }
